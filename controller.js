...
firebase
  .auth()
  .signInWithEmailAndPassword("benrosenberger32@gmail.com", "brrose32")
  .then((user) => {
    // Signed in
    // ...
...
exports.playerProfile = async (req, res, next) => {
  let x = await db
    .ref("/profiles/" + req.params.username)
    .once("value")
    .then((snapshot) => {
      return snapshot.val();
    });
...
exports.updateStats = async (req, res, next) => {
  ...
  const careerFinalCupsMade =
    parseInt(req.params.fc) + parseInt(previous.finalCupsMade);
...
function postUpdatedStatstoServer(
  username,
  cupsMadeGame,
  finalCupsMadeGame,
  shotsGame,
